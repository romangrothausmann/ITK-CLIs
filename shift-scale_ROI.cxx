////program for itkROIShiftScaleImageFilter
...
int DoIt(int argc, char *argv[]){
    ...
    if( argc != 5 + 2*Dimension){
	fprintf(stderr, "4 + 2*Dimension = %d parameters are needed!\n", 5 + 2*Dimension - 1);
	return EXIT_FAILURE;
	}
    ...
    for (i= 0; i < Dimension; i++){
        ROIindex[i]= atoi(argv[5+i]);
    	std::cerr << ROIindex[i] << std::endl;
    	}
    for (i= 0; i < Dimension; i++){
        ROIsize[i]= atoi(argv[5+Dimension+i]);
    	std::cerr << ROIsize[i] << std::endl;
    	}
    typename InputImageType::RegionType ROI(ROIindex, ROIsize);


    typedef itk::ROIShiftScaleImageFilter<InputImageType, OutputImageType> FilterType;
    typename FilterType::Pointer filter= FilterType::New();
    filter->SetInput(input);
    filter->SetDesiredMean(atof(argv[3]));
    filter->SetDesiredStd(atof(argv[4]));
    filter->SetROI(ROI);
