...
//02: based on new template.cxx including SDI
...
int DoIt(int argc, char *argv[]){
    ...
    for (i= 0; i < Dimension; i++){
        ROIindex[i]= atoi(argv[4+i]);
    	std::cerr << ROIindex[i] << std::endl;
    	}
    for (i= 0; i < Dimension; i++){
        ROIsize[i]= atoi(argv[4+Dimension+i]);
    ...
    try{ // stat needs to run to get results even with SDI
        stat->Update();
        }
    catch(itk::ExceptionObject &ex){ 
	std::cerr << ex << std::endl;
    ...
    for (i= 0; i < Dimension - 1; i++){
        ROISBSindex[i]= atoi(argv[4+i]);
    	std::cerr << ROISBSindex[i] << std::endl;
    	}
    for (i= 0; i < Dimension - 1; i++){
        ROISBSsize[i]= atoi(argv[4+Dimension+i]);
